...
In another terminal, install and build the CLI skill first:
...
| `AUTH_TOKEN` | *(empty)* | Set to require `Bearer` token auth on all endpoints (set `BACKEND_AUTH_TOKEN` to the same value in the CLI environment) |
