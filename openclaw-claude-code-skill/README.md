...
Control Claude Code via an HTTP-to-CLI bridge. This CLI provides programmatic access to Claude Code's capabilities including persistent sessions, multi-model proxy support, and tool control.

## Features

- **HTTP-to-CLI Bridge** - Sends commands to a backend API server that spawns Claude Code CLI processes
- **Persistent Sessions** - Maintain context across multiple interactions
- **Multi-Model Proxy** - Route through any OpenAI-compatible endpoint
...
```bash
# Connection
claude-code-skill connect          # Check backend server connectivity
...
# [Not Implemented] The following commands are registered in the CLI
# but the backend endpoints return 501:
# claude-code-skill session-history myproject -n 50   # View history
# claude-code-skill session-pause myproject            # Pause session
# claude-code-skill session-resume-paused myproject    # Resume paused session
# claude-code-skill session-fork myproject exp         # Fork session
# claude-code-skill session-search "bug fix"           # Search sessions
```
...
The session search and history endpoints are not yet implemented in the backend (return 501).

```bash
claude-code-skill sessions -n 20              # List recent sessions (works)
claude-code-skill resume <session-id> "Continue" -d ~/project  # Resume (works)
```
